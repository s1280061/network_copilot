...
## なぜ必要か
ADAS・自動運転ではGPU/NPU搭載の高性能SoCが必須となり、Classic AUTOSARのリアルタイムOSでは対応できません。Adaptive AUTOSARはPOSIX準拠OS上で動き、アプリケーションを「Adaptive Application(AA)」として疎結合で組み合わせる設計を可能にします。
