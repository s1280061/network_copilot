...
ダイオードのショックレー方程式。\(I_S\) は逆方向飽和電流 [A]、\(n\) は理想係数、\(V_T = kT/q\) は熱電圧（室温で約26 mV）です。順方向電圧 \(V\) の増加に対し電流が指数関数的に増えます。
...
$$
I_C = h_{FE}\, I_B, \qquad I_{C,\text{sat}} = \frac{V_{CC}}{R_C}
$$

コレクタ電流 \(I_C\) [A] はベース電流 \(I_B\) を電流増幅率 \(h_{FE}\)（100〜500程度）倍したもの。\(I_C\) が飽和電流 \(V_{CC}/R_C\) に達すると飽和（ON）状態になります。
...
$$
I_D = \frac{1}{2} k\,(V_{GS} - V_{th})^2 \quad (\text{飽和領域})
$$

ゲート電圧 \(V_{GS}\) がしきい値 \(V_{th}\) を超えるとドレイン電流 \(I_D\) [A] が流れます。飽和領域では \((V_{GS}-V_{th})\) の2乗に比例し、\(k\) はプロセス依存の定数（\(\mu C_{ox} W/L\)）です。
...
CMOS回路の動的消費電力 \(P\) [W]。スイッチングのたびに負荷容量 \(C_{\text{load}}\) [F] を充放電するため、電源電圧 \(V_{DD}\) [V] の2乗とクロック周波数 \(f\) [Hz] に比例します。
