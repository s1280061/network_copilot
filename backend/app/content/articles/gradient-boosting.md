...
## 概要
「表形式データで最高精度を出したい」なら、まず試すべきがXGBoost・LightGBMです。勾配ブースティング（Gradient Boosting）は弱い決定木を順番に積み重ねて強力なモデルを作るアンサンブル手法で、Kaggle等のデータコンペで最も頻繁に優勝しています。ランダムフォレストが木を並列で独立に作るのに対し、ブースティングは**直列に誤差を修正していく**点が違い、少ないデータでも高精度を出せます。

## 活用シーン
- **価格予測**: 不動産・中古車・商品価格の表形式特徴量から高精度予測
- **信用リスク**: 金融機関のローンデフォルト予測（Kaggleコンペ常連）
- **顧客離脱**: 大量の行動ログ特徴量を持つ分類タスク
...
1. **学習率と木の数のバランスを間違える** → 学習率を下げる（`0.05`以下）場合は木の数（`n_estimators`）を増やす。Early Stoppingと組み合わせて最適な木の数を自動決定する。
2. **`eval_set` を設定しないと過学習する** → `fit()` に `eval_set=[(X_test, y_test)]` を渡し、`early_stopping_rounds` を設定して過学習を防ぐ。
3. **LightGBMのカテゴリ変数設定漏れ** → LightGBMはカテゴリ変数をネイティブ対応しているが、明示的に `dtype="category"` を設定するか `categorical_feature` パラメータで指定する必要がある。
4. **SHAP値の解釈ミス** → `shap_values[1]` は陽性クラスのSHAP値。「特徴量の値が高いとSHAP値が高い（予測が上がる）」は常に成立しない。SHAPのBeeswarmプロットで色（値の大小）とSHAP値の関係を確認する。
...
- 勾配ブースティング = 弱い木を直列に積み重ねて誤差を逐次修正
- LightGBM は XGBoost より高速・省メモリ（Leaf-wise 成長）で大規模データに強い
- `learning_rate` は低く（0.05以下）、`n_estimators` は多く（1000以上）+ Early Stopping が基本設定
- SHAP値でブラックボックスモデルの予測根拠を説明できる
- Optuna で `n_trials=50〜100` 程度のハイパーパラメータ探索が実用的
