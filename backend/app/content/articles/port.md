...
## 概要
**ポート番号**は、1台の機器内で動く複数の通信を区別するための16ビットの番号(**0〜65535**)です。IPアドレスが「建物の住所」なら、ポートは「部屋番号」に相当します。

## なぜ必要か
1つのECUが同時に複数の通信(診断・SOME/IP・映像など)を扱うとき、どのデータがどのアプリ宛かを区別する必要があります。**ポート番号**がその振り分けを可能にします。
