...
$$
T(n) = \frac{n(n-1)}{2} = O(n^2), \quad T_{\text{best}}(n) = O(n)
$$

二重ループで比較回数は \(O(n^2)\)。ただし交換が起きなければ早期終了し、ソート済み入力では \(O(n)\) です。
...
$$
T_{\text{avg}}(n) = O(n^2), \quad T_{\text{best}}(n) = O(n)
$$

各要素を挿入位置まで後方に比較するため平均 \(O(n^2)\)。ほぼソート済みなら移動が少なく \(O(n)\) に近づきます。
...
配列を半分に分割して再帰し、\(O(n)\) でマージします。最悪・平均ともに \(\Theta(n \log n)\) を保証します。
...
$$
T_{\text{avg}}(n) = 2T(n/2) + O(n) = O(n \log n), \quad T_{\text{worst}}(n) = T(n-1) + O(n) = O(n^2)
$$

分割が均等なら \(O(n \log n)\)。偏ると \(O(n^2)\) ですが、三数中間法でピボットを選ぶと最悪ケースを回避しやすくなります。
...
要素数 \(n\) と値域 \(k\) に対して線形。比較を行わないため \(k\) が小さければ \(O(n \log n)\) の比較ソートより高速です。
